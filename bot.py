...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format, *args):
        pass  # لا نطبع كل Ping من UptimeRobot في الطرفية
...
USERS_DEFAULT     = {"all_users": [], "banned_users": []}
...
def is_banned(chat_id):
    users = load_users()
    return str(chat_id) in users["banned_users"]
...
def load_settings():
    return load_json(SETTINGS_FILE, SETTINGS_DEFAULT)
...
def handle_start(message):
    ...
    if len(args) > 1 and args[1] == 'sarakhni':
        # دخول مباشر لصارحني (من رابط القناة مثلاً) — بدون شرط اشتراك
...
def handle_all_messages(message):
    ...
    settings = load_settings()
    if settings['maintenance'] and not is_admin(chat_id):
    ...
    # حالات الأدمن الخاصة (إذاعة / تذكير) لا تحتاج فحص اشتراك
    if is_admin(chat_id) and state in ('admin_broadcast_wait', 'admin_reminder_text', 'admin_reminder_time'):
        handle_admin_states(chat_id, text, state)
        return

    # صارحني معفاة من شرط الاشتراك، وأي شيء آخر يتطلبه
    needs_subscription = not (text in SARAKHNI_EXEMPT_TEXTS or state == 'sarakhni')
...
def handle_sarakhni(chat_id, text, msg):
    ...
    sarakhni_data["messages"].append({"anonId": anon_id, "text": text, "identity": identity_line})

    USER_STATES[chat_id] = 'idle'
    bot.send_message(chat_id, "✅ وصلت رسالتك بنجاح", reply_markup=MAIN_KEYBOARD)
    record_stat(action='sarakhni')

    # البطاقة الأولى: النص + الرقم المجهول فقط (صالحة للتمرير للقناة)
    content_msg = bot.send_message(
        ADMIN_CHAT_ID,
...
def handle_admin_swipe_reply(message):
    """رد الأدمن على الطالب بالسحب المباشر على رسالة صارحني."""
...
# ============================================================
#  حالات الأدمن (إذاعة / تذكير) — إدخال نص لا بديل عنه بزر
# ============================================================
def handle_admin_states(chat_id, text, state):
    ...
    if state == 'admin_broadcast_wait':
        USER_STATES[chat_id] = 'idle'
        sent, failed = broadcast_text(f"📢 إعلان من الإدارة\n\n{text}")
        ...
        bot.send_message(chat_id, '🕒 حسناً، الآن أرسلي تاريخ ووقت الإرسال بهذا الشكل:\nYYYY-MM-DD HH:MM\n\nمثال:\n2026-09-01 09:00\n\nللإلغاء أرسل: إلغاء')

    elif state == 'admin_reminder_time':
        try:
            when = datetime.strptime(text, '%Y-%m-%d %H:%M')
        except ValueError:
            bot.send_message(chat_id, '⚠️ الصيغة غير صحيحة، استخدمي:\nYYYY-MM-DD HH:MM\nمثال: 2026-09-01 09:00')
            return
...
def handle_callbacks(call):
    ...
    if data == 'adm_home':
        ...
        edit_or_send(call, '📢 أرسلي الآن نص الإعلان الذي تريدين إرساله لكل الطلاب.\n\nللإلغاء أرسل: إلغاء', None)
...
def build_reminders_view():
    ...
    text += '━━━━━━━━━━━━━━━━━━━━\n✍️ أرسلي الآن نص التذكير الجديد الذي تريدين جدولته.\n\nللإلغاء أرسل: إلغاء'
    return text, None

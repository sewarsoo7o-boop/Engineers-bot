...
BOT_USERNAME = 'UOB_Engineers_bot'  # ⚠️ غيّريه إلى يوزر البوت بدون @ (يُستخدم في رابط صارحني)
...
SARAKHNI_DEFAULT  = {"count": 1, "users": {}, "messages": [], "reply_map": {}}
...
def ban_user(chat_id):
    ...
    users.setdefault("ban_history", []).append({"chat_id": uid, "action": "حظر", "when": datetime.now().isoformat()})
    save_json(USERS_FILE, users)
...
def unban_user(chat_id):
    ...
    users.setdefault("ban_history", []).append({"chat_id": uid, "action": "فك حظر", "when": datetime.now().isoformat()})
    save_json(USERS_FILE, users)

def load_settings():
    settings = load_json(SETTINGS_FILE, SETTINGS_DEFAULT)
    if not settings.get("launch_date"):
        settings["launch_date"] = datetime.now().isoformat()
...
MAIN_KEYBOARD.row('📈 عن البوت', '❓ كيف أستخدم البوت')
...
CALC_GUIDE_1 = (
    "🏛️ دليل المواد ورموزها الأكاديمية — المرحلة العامة\n\n"
    "🔹 مواد الفصل الدراسي الأول (Semester 1):\n\n"
    "• الرياضة (1) [ES101] — (12 وحدة)\n"
    "• الفيزياء (1) [ES111] — (12 وحدة)\n"
    "• اللغة الإنجليزية (1) [EL101] — (9 وحدات)\n"
    "• الإحصاء [ST011] — (9 وحدات)\n"
    "• مبادئ الحاسوب [CS011] — (9 وحدات)\n"
    "• اللغة العربية [AL100] — (6 وحدات)"
)
CALC_GUIDE_2 = (
    "─────\n\n"
    "🔹 مواد الفصل الدراسي الثاني (Semester 2):\n\n"
    "• الرياضة (2) [ES102] — (12 وحدة)\n"
    "• الفيزياء (2) [ES112] — (12 وحدة)\n"
    "• الكيمياء العامة [ES146] — (9 وحدات)\n"
    "• اللغة الإنجليزية (2) [EL102] — (9 وحدات)\n"
    "• الرسم الهندسي [ES166] — (6 وحدات)\n\n"
    "💡 ملاحظة: تُمكّنكم هذه الرموز وعدد الوحدات من حساب معدلكم الفصلي والتراكمي بدقة عبر بوت القناة"
)
...
def admin_main_menu():
    ...
    m.add(
        types.InlineKeyboardButton('🔗 رابط صارحني للقناة', callback_data='adm_sk_link'),
    )
    m.add(
...
def handle_all_messages(message):
    ...
    needs_subscription = not (is_admin(chat_id) or text in SARAKHNI_EXEMPT_TEXTS or state == 'sarakhni')
    ...
    if text == '📊 معدلي الحالي':
        ...
        bot.send_message(chat_id, CALC_GUIDE_1)
        bot.send_message(chat_id, CALC_GUIDE_2)
        return
        ...
        bot.send_message(chat_id, build_public_stats_text())
        return
...
def handle_sarakhni(chat_id, text, msg):
    ...
    header = f"😍 لديك صراحة من مجهول {anon_id}\n💬 صارحني UOB Engineers Bot"
    ...
    # البطاقة الثانية: هوية المُرسل، لعلم الإدارة فقط، مع زر حظر سريع
...
def handle_admin_swipe_reply(message):
    ...
    try:
        bot.send_message(target_chat_id, f"📩 رد من الإدارة على رسالتك:\n\n{reply_text}")
...
def show_help(chat_id):
    text = ('❓ دليل الاستخدام\n━━━━━━━━━━━━━━━━━━━━\n'
            ...
            '📈 عن البوت\nتعرّف على أرقام البوت العامة\n━━━━━━━━━━━━━━━━━━━━')
    bot.send_message(chat_id, text)

def build_public_stats_text():
    stats = load_json(STATS_FILE, STATS_DEFAULT)
    users = load_users()
    settings = load_settings()
    try:
        launch_display = datetime.fromisoformat(settings.get('launch_date', '')).strftime('%Y-%m-%d')
    except Exception:
        launch_display = '—'

    out = "📈 عن بوت UOB Engineers\n━━━━━━━━━━━━━━━━━━━━\n"
    out += f"👥 عدد الطلاب المستفيدين: {len(users['all_users'])}\n"
    out += f"🧮 عمليات حساب المعدل حتى الآن: {stats.get('calc_count', 0)}\n"
    out += f"💬 رسائل صارحني المستقبلة: {stats.get('sarakhni_count', 0)}\n"
    out += f"📅 يعمل منذ: {launch_display}\n━━━━━━━━━━━━━━━━━━━━"
    return out
...
def handle_callbacks(call):
    ...
    if data == 'adm_home':
        ...
        edit_or_send(call, f"🔗 رابط صارحني (ضعيه بقناتك):\n{link}", admin_back_button())
...
def send_backup(chat_id):
    ...
    zip_name = f"backup_{datetime.now().strftime('%Y%m%d_%H%M')}.zip"
    try:

...
def handle_all_messages(message):
    try:
        ...
        commands = {}
        if features.get('gpa_current', True):
            commands['📊 معدلي الحالي'] = lambda: (track_feature('📊 معدلي الحالي'), safe_send_message(chat_id, '📊 هذه الميزة تتيح لك حساب معدلك عبر إدخال المواد مباشرة.\n\nاستخدم زر: 🧮 حاسبة المعدل'))
        if features.get('gpa_calc', True):
            commands['🧮 حاسبة المعدل'] = lambda: set_state_and_send(chat_id, 'manual_calc', '🧮 أرسل موادك (الوحدات ثم التقدير):\nمثال:\n12 BB\n9 CC\n\nللإلغاء أرسل: إلغاء')
        if features.get('what_if', True):
            commands['🎯 كم أحتاج لهدف معين'] = lambda: set_state_and_send(chat_id, 'what_if', '🎯 أرسل 4 أرقام في أسطر متتالية:\n1️⃣ التراكمي الحالي\n2️⃣ الوحدات المنجزة\n3️⃣ الوحدات القادمة\n4️⃣ المعدل المستهدف\n\nللإلغاء أرسل: إلغاء')
        if features.get('improve', True):
            commands['🔄 أثر تحسين مادة'] = lambda: set_state_and_send(chat_id, 'improve', '🔄 أرسل 5 أسطر:\n1️⃣ التراكمي الحالي\n2️⃣ مجموع وحداتك\n3️⃣ وحدات المادة\n4️⃣ تقديرها القديم\n5️⃣ تقديرها المتوقع\n\nللإلغاء أرسل: إلغاء')
        if features.get('my_grade', True):
            commands['🏆 ما تقديري؟'] = lambda: set_state_and_send(chat_id, 'my_grade', '🏆 أرسل معدلك فقط (مثال: 3.20)')
        if features.get('balance', True):
            commands['⚖️ موازنة مواد فصلي'] = lambda: set_state_and_send(chat_id, 'balance', '⚖️ أرسل توقعات المواد (الوحدات ثم التقدير)\nمثال:\n12 BB\n9 AA')
        if features.get('sarakhni', True):
            commands['💬 صارحني'] = lambda: set_state_and_send(chat_id, 'sarakhni', '💬 **قسم صارحني**\nأرسل اقتراحك أو استفسارك وسيصل للإدارة بسرية تامة 🔒\n\nاكتب رسالتك الآن 👇\nللإلغاء أرسل: إلغاء')
        ...
        if text in commands:
            commands[text]()
            return
        elif text in ['📊 معدلي الحالي', '🧮 حاسبة المعدل', '🎯 كم أحتاج لهدف معين', '🔄 أثر تحسين مادة', '🏆 ما تقديري؟', '⚖️ موازنة مواد فصلي', '💾 سجل معدلاتي', '💬 صارحني']:
            safe_send_message(chat_id, '⚠️ هذه الميزة معطلة حالياً من قبل الإدارة.')
            return
...
def handle_sarakhni(chat_id, text, msg):
    ...
    USER_STATES[chat_id] = 'idle'
    safe_send_message(chat_id, "✅ وصلت رسالتك للإدارة بنجاح، شكراً لتواصلك!", reply_markup=get_main_keyboard())
    
    # 1. الرسالة الأولى للإدارة: نص الصراحة فقط
    msg_for_channel = f"💬 **رسالة صراحة جديدة (مجهول #{anon_id}):**\n\n{text}"
    safe_send_message(ADMIN_CHAT_ID, msg_for_channel, parse_mode='Markdown')
    
    # 2. الرسالة الثانية للإدارة: تفاصيل الهوية مع أزرار التحكم الفوري
    msg_identity = (

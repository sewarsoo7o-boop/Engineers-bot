...
def handle_all_messages(message):
    ...
    is_sarakhni_action = (text in SARAKHNI_EXEMPT_TEXTS) or (state == 'sarakhni' and text not in FEATURE_KEYS)
...
def handle_manual_calc(chat_id, text, is_balance):
    ...
    for line in lines:
        ...
        if not m:
            # صيغة القالب الجاهز: "12 الرياضة 1: BB"
            m = re.match(r'^(\d+(?:\.\d+)?)\s+.+:\s*([A-Za-z]{1,2})$', norm)
        if not m:
            continue
